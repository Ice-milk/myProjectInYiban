	...
	var c = function() {
		...
		this.init = function(b, c) {
			...
			if(this.el.css({
				...
				var d = function() {
					c.el.css("width", Math.min(Math.round(100 * (c.el.outerWidth() / c.el.parent().outerWidth())), 100) + "%")
				};
				d(), a(window).resize(d)
			}
			...
			}), a.event.special.swipe && this.el.on("swipeleft", c.prev).on("swiperight", c.next)
		}, this.move = function(b, d) {
			var e, f, g;
			this.items.eq(b).length || (b = 0), 0 > b && (b = this.items.length - 1), e = this.items.eq(b), f = {
				height: e.outerHeight()
			}, g = d ? 5 : this.opts.speed, this.ul.is(":animated") || (c.el.find(".dot:eq(" + b + ")").addClass("active").siblings().removeClass("active"), this.el.animate(f, g) && this.ul.animate(a.extend({
				left: "-" + b + "00%"
			}, f), g, function() {
