...
switch ($_GET['action']){ 
    ...
    case "add"://增加操作
		...
		$sql = "insert into ranking(class,name,xueyuan,data,score,rank) values('{$class}','{$name}','{$xueyuan}','{$data}','{$score}','{$rank}')";
        $rw = $pdo->exec($sql);
        if($rw > 0){
            echo "<script>alert('success');window.location='add.php';</script>";
        }else{
